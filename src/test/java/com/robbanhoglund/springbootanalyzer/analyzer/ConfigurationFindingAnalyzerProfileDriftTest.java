...
class ConfigurationFindingAnalyzerProfileDriftTest {
    ...
    @Test
    void flagsOpenInViewSetToTrueExplicitly() {
        ConfigurationAnalysis cfg = config(prop("spring.jpa.open-in-view", "true", null));

        Finding f = byRule(findings(cfg), "SPRING_JPA_OPEN_IN_VIEW");
        assertThat(f).isNotNull();
        assertThat(f.message()).contains("open-in-view");
    }

    @Test
    void flagsOpenInViewSetToTrueInProdProfile() {
        ConfigurationAnalysis cfg = config(prop("spring.jpa.open-in-view", "true", "prod"));
    ...
    @Test
    void doesNotFlagOpenInViewSetToFalse() {
        ConfigurationAnalysis cfg = config(prop("spring.jpa.open-in-view", "false", null));
