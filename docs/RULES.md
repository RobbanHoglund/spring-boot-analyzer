...
| Rule ID | Severity | What it detects | Why it matters | Recommendation |
|---------|----------|-----------------|----------------|----------------|
| `SPRING_MODIFYING_NO_TRANSACTION` | WARNING | Spring Data `@Modifying` query with no `@Transactional` on the method or its class | Without *any* active transaction the query throws `TransactionRequiredException`. The boundary is commonly (and correctly) supplied by a `@Transactional` service caller, which per-file static analysis cannot see — verify a transactional caller exists | Add `@Transactional` to the repository method, or confirm the calling service method is transactional |
...
| `SPRING_TRANSACTIONAL_EXCEPTION_SWALLOWED` | WARNING | `@Transactional` method contains a `catch` block that catches `RuntimeException` or `Exception` without rethrowing (the detector does not verify a write happens in the `try`, so a deliberate catch may be intentional) | Spring's automatic rollback is only triggered by uncaught runtime exceptions; swallowing the exception leaves the database in a partially written state | Either rethrow the exception or call `TransactionAspectSupport.currentTransactionStatus().setRollbackOnly()` explicitly |
