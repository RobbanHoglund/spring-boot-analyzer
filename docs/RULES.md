...
| `SPRING_TRANSACTIONAL_NON_PUBLIC_METHOD` | **ERROR** | `@Transactional` on a `protected` or package-private method | Spring's proxy applies transaction advice only to public methods, so the annotation is silently ignored — partial writes with no rollback | Make the method `public`, or move the boundary to a public method invoked through the proxy |
