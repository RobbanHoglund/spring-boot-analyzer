...
| `SPRING_CORS_CREDENTIALS_WILDCARD` | WARNING | A wildcard origin (`allowedOriginPatterns("*")`/`allowedOrigins("*")`) is combined with `allowCredentials(true)` | Spring reflects any origin back and sends cookies/Authorization headers to it, enabling cross-origin credential theft | Enumerate the exact trusted origins, or drop `allowCredentials` if the API is genuinely public |
...
| `SPRING_JPA_OPEN_IN_VIEW` | WARNING | `spring.jpa.open-in-view` either explicitly `true`, or not set at all in a project with a datasource (Spring Boot defaults it to `true`) | Keeps the Hibernate session open through the entire HTTP request, enabling lazy loading during serialization and masking N+1 queries | Set `spring.jpa.open-in-view=false` and load all required data explicitly in the service layer |
...
| `SPRING_LIQUIBASE_MISSING_CHANGELOG` | WARNING | `liquibase-core` on the classpath but no changelog file at the configured `spring.liquibase.change-log` location (default `classpath:/db/changelog/db.changelog-master.yaml`) | Spring Boot auto-configures Liquibase and fails startup with "Cannot find changelog location" — typically discovered at deploy time | Create the changelog at the expected location, or point `spring.liquibase.change-log` at the actual file |
...
| `SPRING_TRANSACTIONAL_READONLY_WITH_WRITES` | WARNING | A `@Transactional(readOnly = true)` method performs persistence writes (`save`/`delete`/`persist`/`merge`/…) | Read-only sets the Hibernate flush mode to MANUAL, so writes are never flushed (silent data loss) or fail late on a read-only connection | Remove `readOnly = true` from methods that write, or split read and write into separate transactions |
...
| `SPRING_TRANSACTIONAL_CHECKED_EXCEPTION_NO_ROLLBACK` | WARNING | A `@Transactional` method declares a checked exception in its `throws` clause but the annotation has no `rollbackFor` | Spring rolls back only on `RuntimeException`/`Error` by default, so a thrown checked exception commits the partial transaction | Add `rollbackFor` (e.g. `@Transactional(rollbackFor = Exception.class)`), or wrap the checked exception in a `RuntimeException` |
