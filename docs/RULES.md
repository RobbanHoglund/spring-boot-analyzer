...
| `SPRING_JPA_DDL_AUTO_DANGEROUS` | **ERROR** | `spring.jpa.hibernate.ddl-auto` or `spring.datasource.initialization-mode` set to `create` or `create-drop` in a production-oriented profile | Drops and recreates all tables on every startup, destroying all production data | Set to `validate` or `none` in production; manage schema with Flyway or Liquibase |
...
| `SPRING_REACTIVE_API_IN_SERVLET_APP` | INFO | Reactive types (`Mono`, `Flux`, WebFlux imports) found in a project that uses the Servlet/MVC stack | Mixing reactive and blocking code paths causes subtle threading issues and prevents back-pressure from working correctly | Use either the servlet stack or the reactive stack consistently; do not mix `spring-boot-starter-web` and `spring-boot-starter-webflux` |
