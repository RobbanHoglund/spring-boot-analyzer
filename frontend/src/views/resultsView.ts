...
export interface ResultsViewActions {
  ...
  onOpenHttpSource: (sourceFile: string, line: number | undefined, label: string, triggerId: string) => void;
...
function renderResultsOverviewBlock(result: AnalyzeRepositoryResponse, actions: ResultsViewActions): HTMLElement {
  ...
  grid.append(
    ...
    renderSecurityPostureCard(result, actions),
    renderPersistenceSummaryCard(result, actions)
  );
...
function renderTopConcernsCard(result: AnalyzeRepositoryResponse): HTMLElement {
  ...
  if (concerns.length === 0) {
    const body = element('div', { className: 'insight-card-body' });
    body.appendChild(
      element('div', {
  ...
  const list = element('ul', { className: 'top-concern-list' });
  concerns.forEach((concern) => {
    const item = element('li', { className: `top-concern-item top-concern-${concern.severity.toLowerCase()}` });
  ...
  insightCardBody(card, list, { href: '#results-findings', label: 'View all findings' });
  return card;
}

function renderSecurityPostureCard(result: AnalyzeRepositoryResponse, actions: ResultsViewActions): HTMLElement {
  const findings = result.findings ?? [];
  ...
  const securityConfigComponent = (result.detectedComponents ?? []).find((component) => {
  ...
  const healthDetailsExposed = healthFinding != null;
  const secCfgSourceFile = securityConfigComponent?.filePath ?? null;
  const secCfgLabel = securityConfigComponent ? componentDisplayName(securityConfigComponent) : 'Security config';
  const healthSourceFile = healthFinding?.primaryLocation?.filePath ?? healthFinding?.sourceFile ?? null;
  ...
  list.appendChild(renderDetailRow('Spring Security',
    securityPresent ? 'Present' : 'Not detected',
    securityPresent ? 'positive' : 'muted'));

  list.appendChild(renderDetailRow('Security configuration',
  ...
  list.appendChild(renderDetailRow('Weak secret defaults',
    ...
    weakSecretDefaults > 0 ? 'error' : 'positive',
    weakSecretDefaults > 0 ? { kind: 'href', href: '#results-findings', label: 'View findings' } : undefined));

  list.appendChild(renderDetailRow('Raw exception responses',
    String(rawExceptionExposures),
    rawExceptionExposures > 0 ? 'error' : 'positive',
    rawExceptionExposures > 0 ? { kind: 'href', href: '#results-findings', label: 'View findings' } : undefined));

  list.appendChild(renderDetailRow('Health details exposure',
    healthDetailsExposed ? 'Review needed' : 'Not detected',
  ...
  list.appendChild(renderDetailRow('Endpoint authorization model', 'Not confirmed statically', 'muted'));
  insightCardBody(card, list, { href: '#results-http', label: 'View full HTTP surface' });
  return card;
}

function renderPersistenceSummaryCard(result: AnalyzeRepositoryResponse, actions: ResultsViewActions): HTMLElement {
  const findings = result.findings ?? [];
  const dependencies = collectDependencyNotations(result);
  const components = result.detectedComponents ?? [];
  const repositories = components.filter((component) => normalizeComponentType(component) === 'REPOSITORY');
  const entities = components.filter((component) => {
    const ann = (component.annotationNames ?? component.annotations ?? []).map((a) => a.toLowerCase());
    return ann.some((a) => a === 'entity' || a === 'document' || a === 'table');
  });
  ...
  const isJpa = persistenceStyle.includes('JPA');

  const flywayPresent = dependencies.some((d) => d.includes('org.flywaydb:flyway-core'))
    || (result.gradleModelAnalysis?.pluginDeclarations ?? []).some((p) => p.pluginId === 'org.flywaydb.flyway')
    || (result.gradleModelAnalysis?.plugins ?? []).some((p) => defaultText(p.pluginId).includes('flyway'));
  const liquibasePresent = dependencies.some((d) => d.includes('org.liquibase:liquibase-core'));
  const schemaMigration = flywayPresent && liquibasePresent
    ? 'Flyway + Liquibase'
    : flywayPresent
      ? 'Flyway'
      : liquibasePresent
  ...
  const redisPresent = dependencies.some((d) => d.includes('spring-boot-starter-data-redis') || d.includes('io.lettuce:lettuce-core') || d.includes('redis.clients:jedis'));
  const caffeinePresent = dependencies.some((d) => d.includes('com.github.ben-manes.caffeine:caffeine'));
  const ehcachePresent = dependencies.some((d) => d.includes('org.ehcache:ehcache'));
  const cacheStarterPresent = dependencies.some((d) => d.includes('spring-boot-starter-cache'));
  const cacheLayer = redisPresent
    ? 'Redis'
    : caffeinePresent
      ? 'Caffeine'
      : ehcachePresent
        ? 'Ehcache'
        : cacheStarterPresent
          ? 'Spring Cache (generic)'
          : null;

  const properties = result.configurationAnalysis?.properties ?? [];
  const datasourceUrls = properties.filter((p) => p.name === 'spring.datasource.url' || (p.name ?? '').match(/^spring\.datasource\.\w+\.url$/));
  const runtimeDatasourceProp = datasourceUrls.find((p) => {
    const profile = (p.profile ?? 'default').toLowerCase();
    return profile === 'default' || profile === 'prod' || profile === 'production';
  }) ?? datasourceUrls[0];
  const multiDatasource = datasourceUrls.length > 1;
  ...
  const persistencePresent = persistenceStyle !== 'Not confirmed statically'
    ...
    || flywayPresent
    || liquibasePresent;
  const transactionConcerns = findings.filter((f) => normalizeFindingCategory(f.category) === 'TRANSACTION').length;
  ...
  const styleValue = persistencePresent ? persistenceStyle : 'Not confirmed statically';
  list.appendChild(renderDetailRow('Persistence style',
    styleValue,
    persistencePresent && persistenceStyle !== 'Not confirmed statically' ? 'positive' : 'muted'));

  list.appendChild(renderDetailRow('Runtime database',
  ...
  list.appendChild(renderDetailRow('Test database',
    testDatabase ?? 'Not confirmed statically',
    testDatabase ? 'positive' : 'muted',
    testDatabase ? { kind: 'href', href: '#results-configuration', label: 'View configuration' } : undefined));

  list.appendChild(renderDetailRow('Schema migration',
    schemaMigration ?? 'None detected',
    schemaMigration ? 'positive' : 'warning'));

  list.appendChild(renderDetailRow('Cache layer',
    cacheLayer ?? 'None detected',
    cacheLayer ? 'positive' : 'neutral'));

  list.appendChild(renderDetailRow('Repositories',
    repositories.length > 0 ? `${repositories.length} detected` : '0 detected',
    repositories.length > 0 ? 'positive' : 'muted',
    repositories.length > 0 ? { kind: 'href', href: '#results-components', label: 'View repository components' } : undefined));

  if (isJpa) {
    list.appendChild(renderDetailRow('JPA entities',
      entities.length > 0 ? `${entities.length} detected` : '0 detected',
      entities.length > 0 ? 'positive' : 'muted',
      entities.length > 0 ? { kind: 'href', href: '#results-components', label: 'View entity components' } : undefined));
  }

  if (multiDatasource) {
    list.appendChild(renderDetailRow('Multi-datasource', 'Detected', 'warning',
      { kind: 'href', href: '#results-configuration', label: 'View configuration' }));
  }

  list.appendChild(renderDetailRow(
    'Transaction findings',
    transactionConcerns > 0 ? `${transactionConcerns} to review` : 'None',
    transactionConcerns > 0 ? 'warning' : 'positive',
    transactionConcerns > 0 ? { kind: 'href', href: '#results-findings', label: 'View transaction findings' } : undefined));

  insightCardBody(card, list, { href: '#results-components', label: 'View persistence components' });
  return card;
}
...
function renderInboundEndpointsTable(
  ...
  for (const endpoint of visible) {
    ...
    appendCells(row, [
      ...
      httpSourceLinkCell(endpoint.sourceFile, endpoint.line,
        `${endpoint.handlerMethod ?? endpoint.controllerClass ?? 'endpoint'}`, srcId, actions),
...
function renderOutboundEndpointsTable(
  ...
  for (const endpoint of visible) {
    ...
    appendCells(row, [
      ...
      httpSourceLinkCell(endpoint.sourceFile, endpoint.line,
        `${endpoint.clientType ?? 'HTTP client'} → ${endpoint.host ?? endpoint.urlOrTemplate ?? '?'}`, srcId, actions),
...
function renderConfiguredUrlsTable(
  ...
  for (const item of visible) {
    ...
    appendCells(row, [
      ...
      httpSourceLinkCell(item.sourceFile, item.line, item.propertyName ?? 'property', srcId, actions)
    ]);
  }
  block.appendChild(wrapTable(table));
  if (urls.length > 25) {
...
function renderActuatorTable(
  ...
  for (const exposure of visible) {
    ...
    appendCells(row, [
      ...
      httpSourceLinkCell(exposure.sourceFile, exposure.line, exposure.propertyName ?? 'property', srcId, actions),
...
type InsightCardTone = 'concerns' | 'security' | 'persistence';

const INSIGHT_ICONS: Record<InsightCardTone, string> = {
  concerns: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 2 2 16h14Z"/><path d="M9 7.5v3.5"/><circle cx="9" cy="13.5" r="0.9" fill="currentColor" stroke="none"/></svg>`,
  security: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 2 3 4.5v4.8c0 3.2 2.5 6 6 6.7 3.5-.7 6-3.5 6-6.7V4.5Z"/><path d="m6.5 9 1.8 1.8 3.2-3.2"/></svg>`,
  persistence: `<svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><ellipse cx="9" cy="5" rx="5.5" ry="2"/><path d="M3.5 5v8c0 1.1 2.5 2 5.5 2s5.5-.9 5.5-2V5"/><path d="M3.5 9c0 1.1 2.5 2 5.5 2s5.5-.9 5.5-2"/></svg>`,
};

function summaryInsightCard(title: string, description: string, tone?: InsightCardTone): HTMLElement {
  const toneClass = tone ? ` insight-card-${tone}` : '';
  const card = element('div', { className: `summary-card insight-summary-card${toneClass}` });

  const header = element('div', { className: 'insight-card-header' });
  const titleRow = element('div', { className: 'insight-title-row' });
  if (tone) {
    const iconSpan = document.createElement('span');
    iconSpan.className = 'insight-card-icon';
    iconSpan.innerHTML = INSIGHT_ICONS[tone];
    titleRow.appendChild(iconSpan);
  }
  titleRow.appendChild(element('div', { className: 'summary-value insight-summary-title', text: title }));
  header.appendChild(titleRow);
  card.appendChild(header);

  // Description lives in a separate zone below the header so header height is always consistent
  card.appendChild(
    element('div', { className: 'insight-description-zone' },
...
function insightCardBody(card: HTMLElement, content: HTMLElement, footer?: { href: string; label: string }): void {
  const body = element('div', { className: 'insight-card-body' });
  body.appendChild(content);
  card.appendChild(body);
  if (footer) {
    const footerEl = element('div', { className: 'insight-card-footer' });
    footerEl.appendChild(element('a', { className: 'insight-card-footer-link', text: footer.label, attributes: { href: footer.href } }));
...
function renderDetailRow(
  label: string,
  value: string,
  tone: DetailValueTone = 'neutral',
  action?: DetailRowAction
): HTMLElement {
  const toneClass = tone !== 'neutral' ? ` detail-value-${tone}` : '';
  const row = element('div', { className: 'detail-row overview-detail-row' });
  row.appendChild(element('div', { className: 'detail-label', text: label }));
  const valueCell = element('div', { className: `detail-value property-detail-value${toneClass}${action ? ' detail-value-with-action' : ''}` });
  valueCell.appendChild(element('span', { text: value }));
  if (action) {
    if (action.kind === 'href') {
      const link = element('a', {
        className: 'detail-row-action',
        text: 'View →',
        attributes: { href: action.href, title: action.label }
      });
...
function httpSourceLinkCell(
  ...
  const cell = document.createElement('td');
  cell.className = 'http-cell-source';
  if (!sourceFile) {
    cell.textContent = '—';
    return cell;
  }
...
function renderRelatedSignalsSection(signals: RelatedFindingSignal[]): HTMLElement {
  const section = element('div', { className: 'property-detail-section' });
  section.appendChild(element('div', { className: 'property-detail-section-title', text: 'Related signals' }));
  const list = element('ul', { className: 'related-signals-list' });
  for (const signal of signals) {
    const item = element('li', { className: 'related-signal-item' });
    const sev = (signal.severity ?? 'info').toLowerCase();
    item.appendChild(
      element('span', {
        className: `badge badge-${sev} related-signal-badge`,
        text: signal.severity ?? '',
        attributes: { title: severityExplanation(signal.severity ?? '') }
      })
    );
    item.appendChild(
      element('span', { className: 'related-signal-title', text: signal.title || signal.ruleId || '—' })
    );
    if (signal.ruleId) {
      item.appendChild(element('code', { className: 'related-signal-rule-id', text: signal.ruleId }));
    }
    if (signal.evidence?.trim()) {
      item.appendChild(element('div', { className: 'related-signal-evidence muted-text', text: signal.evidence }));
    }
    list.appendChild(item);
  }
  section.appendChild(list);
