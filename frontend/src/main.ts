    ...
    element(
      ...
      element(
        ...
        element(
          ...
          (() => {
            const titleRow = element('div', { className: 'page-header-title-row' });
            const icon = document.createElement('span');
            icon.className = 'page-header-logo';
            icon.setAttribute('aria-hidden', 'true');
            icon.innerHTML = `<svg width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M17 2C8.7 2 2 8.7 2 17c0 4.5 1.9 8.6 4.9 11.5L17 32l10.1-3.5C30.1 25.6 32 21.5 32 17 32 8.7 25.3 2 17 2z" fill="#6DB33F" opacity="0.14"/>
  <path d="M8 27c0-5.5 3.5-10.5 8.5-12.5C21.5 12.5 26 8.5 26.5 3.5 29.5 7 30 12.5 28 18c-2 5.5-7 10-12.5 10C12 28 8 27.5 8 27z" stroke="#6DB33F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
</svg>`;
            titleRow.appendChild(icon);
            titleRow.appendChild(element('h1', { text: 'Spring Boot Analyzer' }));
            return titleRow;
          })(),
          element('p', { text: 'Clone and analyze Spring Boot repositories without starting the application.' })
        ),
        renderThemeControl()
      )
    )
    ...
    shell.appendChild(
      renderAnalyzeView(
        ...
        },
        {
          ...
          onOpenHttpSource: (sourceFile, line, label, triggerId) => {
            void openHttpSourceModal(sourceFile, line, label, triggerId);
          },
          onCloseFindingCode: () => {
...
async function openHttpSourceModal(sourceFile: string, line: number | undefined, label: string, triggerId: string): Promise<void> {
  const analysisId = state.result?.analysisId ?? state.result?.workspaceId ?? null;
  if (!analysisId) {
    return;
  }
  const language = sourceFile.endsWith('.java') ? 'java'
    : sourceFile.endsWith('.yml') || sourceFile.endsWith('.yaml') ? 'yaml'
    : sourceFile.endsWith('.properties') ? 'properties'
    : 'text';
  const occurrence: FindingCodeOccurrence = {
    key: `${sourceFile}:${line ?? 0}`,
    label,
    summary: label,
    location: {
      filePath: sourceFile,
